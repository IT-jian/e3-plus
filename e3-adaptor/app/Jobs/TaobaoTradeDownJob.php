...
class TaobaoTradeDownJob extends Job
{
    ...
    /**
     * TaobaoTradeDownJob constructor.
     *
     * @param $trade
     */
    ...
    public function handle()
    {
        ...
        $taobaoTrade = TaobaoTrade::select($taobaoTradeFields)->where('tid', $tid)->first();
        if (isset($taobaoTrade) && strtotime($taobaoTrade['modified']) >= strtotime($this->trade['jdp_modified'])) { // 最后更新时间，没变则返回成功
            Log::debug("处理交易号[{$tid}]--> 完成：最后更新时间一致");
            $this->runSuccess();
            return true;
        }
        // 查询 rds 的最新状态
        $trade = \App\Models\TaobaoCloudTrade::where('tid', $tid)->first();
        $tradeDetail= $trade['jdp_response'];
        $tradeDetail = $tradeDetail['trade_fullinfo_get_response']['trade'];
        if (empty($tradeDetail['tid'])) {
            Log::debug("处理交易号[{$tid}]--> 完成：rds 信息不完整");
            $this->runSuccess();
            return true;
        }
        ...
        if (empty($shangdian)) { // 不是本系统的订单
            DB::update("update get_taobao_trade set sync_status=3 WHERE tid= '{$tid}' AND sync_status=9");
            return true;
        }
        ...
        try {
            ...
            DB::rollBack();
            Log::debug("处理交易号[{$tid}]--> 失败：" . $e->getMessage());
