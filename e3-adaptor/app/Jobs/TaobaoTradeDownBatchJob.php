...
class TaobaoTradeDownBatchJob extends Job
{
    private $data;
    private $result;
    private $startAt;
    ...
    public function handle()
    {
        ...
        $this->log_time_tid_arr = [];
        \Log::info('开始执行', [$this->startAt]);
        $this->logAt = microtime(true);
        $shangdianMap = $this->getShangdianAppNickMap();
        foreach (array_chunk($this->data, 100) as $tid_arr) {
            $this->failTidArr = [];
            ...
            foreach ($tradeCloudMap as $tradeCloud) {
                ...
                // 转入本地
                try {
                    $this->log_time_tid("事务开始", $tid);
                    \DB::beginTransaction();
                    $tradeDetail = $this->preprocess($tradeDetail);
                    $this->log_time_tid("all 格式化", $tid);
                    ...
                    if (isset($taobaoTrade) && !empty($taobaoTrade) && empty($taobaoTrade['order_id']) && in_array($taobaoTrade['is_tran_success'], array(0, 1))) {
                        ...
                        $this->log_time_tid("format trade", $tid);
                        $insertTrade = $this->parseTaobaoTrade($tradeDetail);
                        // $ttid = \DB::table('taobao_trade')->insertGetId($insertTrade);
                        $this->insert_multi('taobao_trade', [$insertTrade]);
                        $ttid = \DB::getPdo()->lastInsertId();
                        $this->log_cost('trade insert');
                        unset($insertTrade);
                        $this->log_time_tid("insert trade", $tid);
                        ...
                        $this->log_time_tid("format items", $tid);
                        if (!empty($insertItems)) {
                            // $result = \DB::table('taobao_items')->insert($insertItems);
                            $result = $this->insert_multi('taobao_items', $insertItems);
                            unset($insertItems);
                            if (!$result) {
                                throw new Exception('插入taobao_items表失败');
                            }
                        }
                        $this->log_time_tid("insert items", $tid);
                        ...
                        if ($tradeDetail['promotions']) {
                            ...
                            $this->log_time_tid("format promotions", $tid);
                            // $ret = \DB::table('taobao_promotion')->insert($promotions);
                            $ret = $this->insert_multi('taobao_promotion', $promotions);
                        ...
                        $this->log_time_tid("insert promotions", $tid);
                        // 调用任务链
                ...
                $this->log_time_trade($tid, $starttime);
            }
        ...
        \Log::info('total:['.count($this->data) . ']', [(microtime(true) - $this->startAt) * 1000, Date('Y-m-d H:i:s'), Date('Y-m-d H:i:s', $this->startAt)]);
        foreach ($this->log_time_tid_arr as $string => $arr) {
            $total = count($arr);
            $avg = 0;
            if ($total) {
                $avg = round(array_sum(array_values($arr))/$total, 3);
            }
            $msg = $string . " total:[{$total}]，avg: [{$avg}]";
            \Log::info($msg);
        }
        unset($this->log_time_tid_arr);
        unset($existTradeMap, $tradeCloudMap, $tradeDetail);

        return true;
    }

    private function log_time_tid($string, $tid)
    {
        $current = microtime(true);
        $cost = round(($current - $this->logAt) * 1000, 3);
        $this->logAt = $current;
        $this->log_time_tid_arr[$string][$tid] = $cost;

        return;
    }

    private function log_time_trade($tid, $from = 0)
    {
        $current = microtime(true);
        $cost = round(($current - $from) * 1000, 3);
        $this->log_time_tid_arr['单个总计'][$tid] = $cost;
        return;
    }
