...
class TaobaoTradeDownTask extends Task
{
    private $data;
    private $result;
    private $startAt;
    ...
    {
        $this->logs = [];
        $this->startAt = $this->logAt = microtime(true);
        $tid = $this->data['tid'];
        // \Log::info(__METHOD__, ['start', microtime(true)]);
        $taobaoTradeFields = ['ttid', 'tid', 'is_tran_success', 'order_id', 'step_trade_status', 'modified'];
        $taobaoTrade = TaobaoTrade::select($taobaoTradeFields)->where('tid', (string)$tid)->first();
        $this->log_cost('select taobao trade');
        if (isset($taobaoTrade) && !empty($taobaoTrade['modified']) && strtotime($taobaoTrade['modified']) >= strtotime($this->data['jdp_modified'])) { // 最后更新时间，没变则返回成功
            $this->runSuccess("处理交易号[{$tid}]--> 完成：最后更新时间一致");
            return true;
        }
        // 查询 rds 的最新状态
        $trade = \App\Models\TaobaoCloudTrade::where('tid', (string)$tid)->first(['jdp_response']);
        $this->log_cost('select rds trade');
        $tradeDetail= $trade['jdp_response'];
        $tradeDetail = $tradeDetail['trade_fullinfo_get_response']['trade'];
        if (empty($tradeDetail['tid'])) {
            $this->runSuccess("处理交易号[{$tid}]--> 完成：rds 信息不完整");
            return true;
        }
        ...
        $this->log_cost('transform rds trade');
        $shangdian = $this->tradeShangdian($tradeDetail);
        $this->log_cost('select shangdian');
        if (empty($shangdian)) { // 不是本系统的订单
            $this->runFail('不是本系统的订单');
            return true;
        }
        ...
        try {
            ...
            if (isset($taobaoTrade) && empty($taobaoTrade['order_id']) && in_array($taobaoTrade['is_tran_success'], array(0, 1))) {
                $updateTrade = $this->parseTaobaoTrade($tradeDetail);
                $result = \DB::table('taobao_trade')->where('tid', $tid)->update($updateTrade);
                $this->log_cost('trade updated');
                unset($updateTrade);
                if (false === $result) {
                ...
                $this->log_cost('trade item updated');
                ...
                if (!empty($insertItems)) {
                    ...
                    $this->log_cost('trade item insert');
                    if (!$result) {
    ...
    public function runSuccess($msg = 'good！')
    {
        $tid = $this->data['tid'];
        $this->result = true;
        \Log::info(__METHOD__, [$tid, (microtime(true) - $this->startAt) * 1000, $this->logs, $msg]);
        return \DB::update("update get_taobao_trade set sync_status=1 WHERE tid= '{$tid}'");
    }

    public function runFail($msg = 'bad！')
    {
        $tid = $this->data['tid'];
        $this->result = false;
        \Log::info(__METHOD__, [$tid, (microtime(true) - $this->startAt) * 1000 . ' ms', $this->logs, $msg]);
        return \DB::update("update get_taobao_trade set sync_status=2 WHERE tid= '{$tid}'");
    }
